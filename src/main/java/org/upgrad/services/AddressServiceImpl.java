...
public class AddressServiceImpl implements AddressService{
    ...
    @Override
    public Integer addAddress(Address address) { return addressRepository.addAddress(address.getFlatNumber(), address.getLocality(), address.getCity() , address.getZipcode() , address.getState().getId()); }
    @Override
    ...
    {
        List<Address> userPermAddList = new ArrayList<>();
        ...
        if( permAddressIdList.iterator().hasNext() )
        {
            for (Integer addressId: permAddressIdList ) {
                ...
                States state = stateRepository.getStatebyId(address.getState().getId());
                Address resp = new Address(address.getId(), address.getFlatNumber(), address.getLocality(), address.getCity(), address.getZipcode(), state);
                userPermAddList.add(resp);
            }
        }
        return userPermAddList;
    }
}